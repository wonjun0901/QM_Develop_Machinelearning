...
edges2 = sobel(Data1)
edges3 = feature.canny(Data2, sigma=1, low_threshold=100, high_threshold=180)
#fill_edges = ndi.binary_fill_holes(edges2)


markers = np.zeros_like(Data2)

markers[edges2 < 0.01] = 1
markers[edges2 > 0.15] = 2
...
#axes[0].imshow(edges1, cmap=plt.cm.gray, interpolation='nearest')
#axes[0].contour(segmentation, [0.5], linewidths=2, colors='y')
#axes[1].imshow(edges3, interpolation='nearest')
...
ax1.imshow(edges2, cmap=plt.cm.gray, interpolation='nearest')
ax1.axis('off')
ax2.imshow(markers, cmap=plt.cm.gray)
ax2.axis('off')
